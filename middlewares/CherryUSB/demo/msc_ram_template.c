...
void usbd_msc_get_cap(uint8_t lun, uint32_t *block_num, uint16_t *block_size) {
    *block_num = (uint32_t) ((uint64_t) SDCardInfo.CardCapacity / 512);
    *block_size = BLOCK_SIZE;
}

int usbd_msc_sector_read(uint32_t sector, uint8_t *buffer, uint32_t length) {
    uint32_t count = length / 512;
    if (count > 1) {
        SD_ReadMultiBlocks(buffer, sector * 512, 512, count);
    } else {
...
int usbd_msc_sector_write(uint32_t sector, uint8_t *buffer, uint32_t length) {
    ...
    {
        SD_WriteMultiBlocks(buffer, sector * 512, 512, count);
    } else {
        SD_WriteBlock(buffer, sector * 512, 512);
    }
    /* Check if the Transfer is finished */
